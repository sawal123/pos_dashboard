@props([
    'mode' => 'no-data-cash', // 'no-data-cash', 'no-data-expense', 'no-data', 'no-results-cash', 'no-results-expense'
])

@if($mode === 'no-data-cash' || $mode === 'no-data')
...
@elseif($mode === 'no-results-cash')
    <div id="cashLedgerFilterEmptyState" class="hidden rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-xs p-8 text-center space-y-3">
...
@elseif($mode === 'no-results-expense')
    <div id="expenseFilterEmptyState" class="hidden rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-xs p-8 text-center space-y-3">
        <div class="w-12 h-12 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-400 mx-auto flex items-center justify-center">
            <i data-lucide="search-x" class="w-6 h-6"></i>
        </div>
        <div class="max-w-xs mx-auto space-y-1">
