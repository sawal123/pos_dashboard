...
@php
    $managedRoles = \App\Models\Business::MANAGED_ROLES;
    ...
    $roleBadgeClass = function (string $category): string {
        return match ($category) {
            ...
            'cashier' => 'bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800',
        ...
        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
            @foreach($users as $user)
                <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td class="px-4 py-3">
                        @if(in_array($user['role_raw'] ?? '', $managedRoles, true))
                            <div class="flex items-center justify-end gap-2">
                                <button
                                    type="button"
                                    data-change-role
                                    data-role-url="{{ route('users.members.role.update', $user['id']) }}"
                                    data-member-name="{{ $user['name'] }}"
                                    data-current-role="{{ $user['role_raw'] }}"
                                    class="inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl border border-indigo-200 dark:border-indigo-900 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 font-semibold text-xs transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
                                >
                        ...
                        @else
                            <div class="text-right text-xs text-slate-400 dark:text-slate-500">—</div>
                        @endif
                    </td>
                </tr>
            @endforeach
